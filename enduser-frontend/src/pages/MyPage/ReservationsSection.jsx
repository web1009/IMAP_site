...
const getMyReservations = async () => {
  try {
    ...
    console.error('[ReservationsSection] 예약 목록 조회 에러:', error);
    return [];
  }
};
...
const getClosingSoonSchedules = async () => {
  ...
  if (response.data.resultCode === 'SUCCESS') return response.data.data;
...
function ReservationsSection({ isVisible = false }) {
  const { reservationsData, setReservationsData } = useMyPageData();
  ...
  useEffect(() => {
    ...
    const fetchReservations = async () => {
      const hasCache = reservationsData.length > 0;
      ...
      try {
        ...
        setReservationsData(transformed);
      } catch (error) {
        console.error('[ReservationsSection] 예약 목록 조회 실패', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  useEffect(() => {
    ...
    const fetchUrgentClasses = async () => {
      ...
      try {
        ...
        console.error('[ReservationsSection] 마감임박 수업 조회 실패', error);
      } finally {
                    ...
                    ? reservations
                        ...
                        .map(r => (
                          <tr key={r.id}>
                            <td>
                              {r.date}
                              {r.time && <div className="text-muted" style={{ fontSize: '0.875rem' }}>{r.time}</div>}
                            </td>
                            <td>{r.branchName}</td>
                            <td>{r.productName}</td>
                            <td>{r.trainerName || '-'}</td>
